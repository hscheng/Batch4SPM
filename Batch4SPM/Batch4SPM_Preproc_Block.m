function Batch4SPM_Preproc_Block()
...
%% function to setup preproc parameters
    para_dlg = inputdlg({'TR',...
        ...
        [1 50;1 50;1 50],{'2',num2str([8 8 8]),'1'}) ;

    preproc_setup.tr = str2num(para_dlg{1});

    preproc_setup.fwhm = str2num(para_dlg{2});
    ...
    preproc_setup.rgm = str2num(para_dlg{3});
    % select run modules
    [ run_modules,~] =listdlg('ListString',{'Realigh_Estimate&Reslice','Normalise_Estimate&Write','Smooth'},...
    'Name','Run Modules','SelectionMode','Multiple',...
    'ListSize',[200 100],'InitialValue',1:3);
...
Dir_Realign = fullfile(WorkDir,'FunImg_Step1_Realign',SubID);
mkdir(Dir_Realign);

Dir_Normalize = fullfile(WorkDir,'FunImg_Step2_Normalize',SubID);
mkdir(Dir_Normalize);

Dir_Smooth = fullfile(WorkDir,'FunImg_Step3_Smooth',SubID);
mkdir(Dir_Smooth);
...
runlist_raw = dir(fullfile(SubDataPath,'run*'));
run_num = length(runlist_raw);
% loop the runs
for ii = 1:run_num

    run_id = runlist_raw(ii,1).name;
    ...
    for kk = 1:scan_num
        scans{kk,1} = [fullfile(SubDataPath,run_id,scanslist(kk).name),',1'];
    end
...
clear matlabbatch;
% move file to New Folders
mean_file = fullfile(SubDataPath,runlist_raw(1,1).name,'meana*.*');
...
for ii = 1:run_num
    run_id = runlist_raw(ii,1).name;
    % Calculate FD
    run_rp_list = dir(fullfile(SubDataPath,run_id,'rp*.txt'));
    run_rp_path = fullfile(SubDataPath,run_id,run_rp_list.name);
...
%         save(fullfile(Dir_rp_Report,'1mm_cutoff',[file_id,'_1mm.txt']),'trial_mark1','-ascii');
 ...
 % **************************** rp_Report [End] ***************************   
    ...
    realigned_file = fullfile(SubDataPath,run_id,'ra*.nii');
    rp_file = fullfile(SubDataPath,run_id,'rp*.txt');
...
for ii = 1:run_num
    ...
    scanslist = dir(fullfile(Dir_Realign,run_id,'r*.nii'));
    run_tr_num = length(scanslist);
    run_scans = cell(run_tr_num,1);
    for kk = 1:run_tr_num
...
for ii = 1:run_num
    ...
    run_id = runlist_normalize(ii,1).name;
    scanslist = dir(fullfile(Dir_Normalize,run_id,'wr*.nii'));
